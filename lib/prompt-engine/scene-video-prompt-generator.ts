...
- 面部表情和情绪状态
- 角色之间的空间关系和互动
- 台词与口型（重要）：如剧情含对白，必须逐条写明说话角色、台词原文（用引号）、说话时的口型开合、语气和表情变化（供即梦/可灵的「对口型」或台词驱动功能使用）；无对白的场景则明确标注"本场景无对白，角色以肢体语言和表情传达情绪"

三、镜头运动与转场
- 开场镜头描述（景别、角度、运动方式：推/拉/摇/移/跟/环绕）
- 镜头之间的转场过渡（重要）：必须逐对描述相邻镜头间的转场，每处转场写明三要素——①转场方式（硬切/叠化/匹配剪辑/遮挡转场等）②视觉衔接元素（前后画面借以衔接的共同物，如屏幕光闪、手部动作、视线方向）③节奏变化（突然/舒缓/加速）
...
七、参考图与首帧引用建议（图片通过平台上传入口引用）
- 基于上下文中提供的镜头图片清单，明确建议哪张图作为视频首帧（frame 1，通常为开场镜头的画面）
- 如平台支持多图参考（如即梦的参考图、可灵的尾帧），逐一说明每张参考图对应的镜头编号和用途
- 角色定妆照/场景参考图如需保持人物与场景一致性，说明其应作为参考图上传
- 注明格式示例："【图片引用】首帧：镜头1画面；参考图：陈默定妆照（保持人物一致性）、大学宿舍场景参考图（保持场景一致性）"
...
- 镜头运动要有明确的起止和节奏
- 相邻镜头之间必须有明确的转场过渡描述（方式 + 衔接元素 + 节奏）
- 如剧情有对白，第二部分必须包含台词原文与口型描述
